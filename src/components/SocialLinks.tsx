...
const socials = [
  { key: 'discord', icon: icons.discord, label: 'Discord', color: 'hover:text-[#5865F2]' },
  { key: 'youtube', icon: icons.youtube, label: 'YouTube', color: 'hover:text-[#FF0000]' },
  { key: 'github', icon: icons.github, label: 'GitHub', color: 'hover:text-white' },
  { key: 'twitter', icon: icons.twitter, label: 'Twitter', color: 'hover:text-[#1DA1F2]' },
];
...
export default function SocialLinks({ links }: { links: SocialLinksData }) {
  ...
  return (
    <div className="flex items-center justify-center gap-6 mt-8 opacity-0 animate-[fadeSlideUp_0.6s_ease_forwards]">
      {socials.map(({ key, icon, label, color }) => {
        ...
        return (
          <a
            ...
            title={label}
            className={`text-2xl text-white/40 ${color} transition-all duration-300 hover:scale-110 active:scale-90`}
          >
            {icon}
          </a>
        );
