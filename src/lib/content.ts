...
export function incrementProjectValue(content: SiteContent): SiteContent {
  const now = Date.now();

  if (!content.projectInfo.nextUpdate || now >= content.projectInfo.nextUpdate) {
    const increment = Math.floor(Math.random() * 9999001) + 10000;
    content.projectInfo.value += increment;
    content.projectInfo.lastUpdate = now;

    const nextInterval = (Math.floor(Math.random() * 60) + 60) * 60 * 1000;
...
export function incrementMemberCount(content: SiteContent): SiteContent {
  const now = Date.now();

  if (!content.memberNextUpdate || now >= content.memberNextUpdate) {
    const increment = Math.floor(Math.random() * 5) + 1;
    content.memberCount += increment;
